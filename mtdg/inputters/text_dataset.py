...
from mtdg.utils.tokenizer import Tokenizer
...
def get_fields(opt):
    fields = {}
    fields["conversation"] = Field(init_token=BOS_TOKEN, eos_token=EOS_TOKEN, pad_token=PAD_TOKEN,
                                   fix_length=opt.max_seq_length, fix_conversation_turn=opt.max_turn_length)
    fields["turn"] = Field(use_vocab=False, sequential=False, dtype=torch.long,
                           fix_conversation_turn=opt.max_turn_length)
    fields["length"] = Field(remain_list=True, use_vocab=False, sequential=False, dtype=torch.long,
                             fix_length=opt.max_seq_length, fix_conversation_turn=opt.max_turn_length)
    fields["indices"] = Field(use_vocab=False, sequential=False, dtype=torch.long,
                              fix_conversation_turn=opt.max_turn_length)
    return  fields
...
def load_fields_from_vocab(vocab, opt):
    """
...
def get_dialog_path_list(corpus_file, dialogs_dir):
...
def read_ubuntu_file(corpus_file, dialogs_dir,
                     min_turn=3, max_turn=10,
                     min_seq=0, max_seq=30,
                     n_workers=1):

    # List of dialogs (tsv)
    dialog_path_list = get_dialog_path_list(corpus_file, dialogs_dir)
    ...
    with Pool(n_workers) as pool:
        conversations = list(tqdm(pool.imap(read_and_tokenize, dialog_path_list),
    ...
    for idx, conv in enumerate(conversations):
        conv_dict = {"conversation": conv, "turn": len(conv), "length": [len(sent) for sent in conv], "indices": idx}
        conv_dicts.append(conv_dict)
    return conv_dicts


def read_ubuntu_convs(corpus_file,
                     min_turn=3, max_turn=10,
                     min_seq=0, max_seq=30,
                     n_workers=1):
    conv_dicts = []
    with codecs.open(corpus_file, "r", "utf-8") as corpus_file:
        for i, line in enumerate(corpus_file):
            utterances = line.strip().split("__eot__")
            if len(utterances[-1]) == 0 or utterances[-1] == "":
                utterances = utterances[:-1]
            if max_turn:
                utterances = utterances[-max_turn:]
            example, lengths = [], []
            ...
            conv_dict = {"conversation": example, "turn": len(example), "length": [len(sent) for sent in example],
                         "indices": i}
            conv_dicts.append(conv_dict)
    return conv_dicts